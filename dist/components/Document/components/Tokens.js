...
const renderText = (tokens, text_fields, containerRef) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (tokens[i].paragraph !== paragraph_nr) {
      if (paragraph.length > 0) {
        field.push(renderParagraph(getLayout(field_name), paragraph_nr, paragraph, tokens[i].paragraph !== paragraph_nr));
      }
    ...
    if (tokens[i].field !== field_name) {
      if (field.length > 0) text["text"].push(renderField(getLayout(field_name), i + "_" + field_name, field, field_name));
      field = [];
    }
  ...
  if (field.length > 0) text["text"].push(renderField(layout, "last_" + field_name, field, field_name));
  return text;
};
