...
var _refNavigation = require("lib/functions/refNavigation");
  ...
  if (space) {
    ...
    if (tokens[mover.position].section !== tokens[newPosition].section) {
      if (newPosition > mover.position) {
        for (let i = newPosition; i >= mover.position; i--) if (tokens[i].section === tokens[mover.position].section) {
          newPosition = i;
          break;
        }
      } else {
  ...
  if (tokens[newSelection[0]].section === tokens[index].section) {
    ...
    if (index > newSelection[0]) {
      for (let i = index; i >= newSelection[0]; i--) {
        if (tokens[newSelection[0]].section === tokens[i].section) newSelection = [newSelection[0], i];
      }
    } else {
      for (let i = index; i <= newSelection[0]; i++) {
        if (tokens[newSelection[0]].section === tokens[i].section) newSelection = [newSelection[0], i];
