...
var _AnnotationEvents = require("./AnnotationEvents");
...
const AnnotateNavigation = _ref => {
  ...
  const [currentToken, setCurrentToken] = (0, _react.useState)({
    i: null
  });
  ...
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, /*#__PURE__*/_react.default.createElement(AnnotationPopup, {
    tokens: tokens,
    ...
    onlyFirst: false
  }), disableAnnotations ? null : /*#__PURE__*/_react.default.createElement(_AnnotationEvents.AnnotationEvents, {
    tokens: tokens,
    annotations: annotations,
    currentToken: currentToken,
    setCurrentToken: setCurrentToken,
...
const showAnnotations = (tokens, annotations, variableMap, editMode) => {
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (!((_token$ref = token.ref) !== null && _token$ref !== void 0 && _token$ref.current)) continue;
    let tokenAnnotations = allowedAnnotations(annotations === null || annotations === void 0 ? void 0 : annotations[token.index], variableMap);
...
const allowedAnnotations = (annotations, variableMap) => {
  ...
  if (annotations && variableMap) {
    annotations = _objectSpread({}, annotations);
    ...
    for (let id of Object.keys(annotations)) {
      ...
      if (!codeMap[code] || !codeMap[code].active || !codeMap[code].activeParent) if (!code === "EMPTY") delete annotations[id];
    }
  }
...
const annotateToken = (token, annotations, variableMap) => {
  ...
  for (let id of Object.keys(annotations)) {
    ...
    if (annotation.span[0] === annotation.index) {
      nLeft++; //colors.pre.push("#ffffff50");
    } else colors.pre.push(color);

    if (annotation.span[1] === annotation.index) {
      nRight++; //valueColors.post.push("#ffffff50");
  ...
  const textColor = (0, _tokenDesign.getColorGradient)(colors.text);
  const preColor = allLeft ? "white" : (0, _tokenDesign.getColorGradient)(colors.pre);
  const postColor = allRight ? "white" : (0, _tokenDesign.getColorGradient)(colors.post);
  setTokenColor(token, preColor, textColor, postColor); //setTokenLabels(token, ["test", "this"]);
...
const showSelection = (tokens, selection) => {
  for (let token of tokens) {
    ...
    if (!((_token$ref2 = token.ref) !== null && _token$ref2 !== void 0 && _token$ref2.current)) continue;
    ...
    let [from, to] = selection; //if (to === null) return false;
...
const AnnotationPopup = /*#__PURE__*/_react.default.memo(_ref2 => {
  var _tokens$currentToken$2;
  ...
  const [refresh, setRefresh] = (0, _react.useState)(0);
  (0, _react.useEffect)(() => {
    var _tokens$currentToken$;

    if (!(tokens !== null && tokens !== void 0 && (_tokens$currentToken$ = tokens[currentToken.i]) !== null && _tokens$currentToken$ !== void 0 && _tokens$currentToken$.ref) || !(annotations !== null && annotations !== void 0 && annotations[tokens[currentToken.i].index]) || !variableMap) {
      setContent(null);
      setRefresh(0);
      return null;
    }
    ...
    const list = ids.reduce((arr, id, i) => {
      ...
      arr.push( /*#__PURE__*/_react.default.createElement(_semanticUiReact.List.Item, {
        key: i,
        style: {
          backgroundColor: (0, _tokenDesign.getColor)(value, codeMap),
          padding: "0.3em"
        },
        onMouseOver: () => setCurrentToken({
          i: null
        })
      }, /*#__PURE__*/_react.default.createElement("b", null, variable), ": " + value));
      return arr;
    }, []);
    setContent( /*#__PURE__*/_react.default.createElement(_semanticUiReact.List, null, list));
    setRefresh(0);
  ...
  return /*#__PURE__*/_react.default.createElement(_semanticUiReact.Popup, {
    ...
    context: tokens === null || tokens === void 0 ? void 0 : (_tokens$currentToken$2 = tokens[currentToken.i]) === null || _tokens$currentToken$2 === void 0 ? void 0 : _tokens$currentToken$2.ref,
    basic: true,
    hoverable: false,
    position: "top left",
    open: true,
...
var _default = AnnotateNavigation;
