...
var _compromiseParagraphs = _interopRequireDefault(require("compromise-paragraphs"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_compromise.default.extend(_compromiseParagraphs.default);
/**
      ...
      for (let sent = 0; sent < t[par].sentences.length; sent++) {
        for (let sent2 = 0; sent2 < t[par].sentences[sent].length; sent2++) {
          ...
          for (let term = 0; term < t[par].sentences[sent][sent2].terms.length; term++) {
            ...
            if (text_field.unit_start != null && token.offset.start + offset >= text_field.unit_start) unit_started = true;
            if (text_field.unit_end != null && token.offset.start + offset > text_field.unit_end) unit_ended = true;
            const tokenobj = {
              section: section,
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (tokens[i].post == null) {
      if (tokens[i].offset != null && tokens[i].length != null) {
        tokens[i].post = i < tokens[i].length - 1 ? " ".repeat(Math.max(0, tokens[i + 1].offset - tokens[i].offset - tokens[i].length)) : "";
      } else {
        tokens[i].post = " ";
      }
    }
    ...
    if (i < tokens.length - 1) totalLength = totalLength + (((_tokens$pre = tokens[i + 1].pre) === null || _tokens$pre === void 0 ? void 0 : _tokens$pre.length) || 0);
    ...
    if (i < tokens.length - 1) {
      if (!tokens[i].section || tokens[i].section === tokens[i + 1].section) {
        ...
        if (tokens[i + 1].offset && tokens[i + 1].offset < tokens[i].offset + totalLength) {
          alert("Invalid token position data. The length of \"".concat(tokens[i].pre + tokens[i].text + tokens[i].post, "\" on position ").concat(tokens[i].offset, " exeeds the offset/start position of the next token"));
          return null;
        }
      }
...
exports.importTokens = importTokens;

const importTokenAnnotations = (tokens, codes) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let annotation of tokens[i].annotations) {
      ...
      if (codeTracker[annotation.name] == null) codeTracker[annotation.name] = {
        index: i,
      ...
      if (codeTracker[annotation.name].value === annotation.value) {
        codeTracker[annotation.name].length = tokens[i].offset + tokens[i].length - codeTracker[annotation.name].offset;
...
exports.importTokenAnnotations = importTokenAnnotations;

const tokensColumnToRow = tokens => {
